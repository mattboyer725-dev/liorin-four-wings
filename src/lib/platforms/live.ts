...
export async function fetchNdjson<T>(url: string, ms = 8000): Promise<T[]> {
  const res = await fetch(url, {
...
type LichessUser = {
  ...
  createdAt?: number;
  count?: { all?: number; rated?: number; win?: number; loss?: number; draw?: number; playing?: number };
};
...
/** Public monsters Liorin will not sit with. The gauntlet field engine is a sparring dummy, not Lc0. */
export const MONSTER_BOT =
  /maia|sargon|leela|stockfish|lynx|raspfish|resolute|duchess|openingsbot|knightodds|leelaknight/i;
...
export async function fetchOnlineBots(nb = 40): Promise<LiveBot[]> {
  ...
  return rows
    ...
    .filter((u) => !MONSTER_BOT.test(u.username))
    .map((u) => ({
      id: u.id,
      username: u.username,
      title: u.title ?? "BOT",
