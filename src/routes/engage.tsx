...
import { chooseFieldMove, chooseMoveFrom, setEngineSalt } from "@/lib/chess/engine";
...
import { MONSTER_BOT } from "@/lib/platforms/live";
...
function EngagePage() {
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(() => {
      ...
      const move = white ? chooseMoveFrom(board, "cold") : chooseFieldMove(board);
      if (!move) return;
      board.move(move);
  ...
  const field = (snap.data?.onlineBots ?? []).filter((b) => !MONSTER_BOT.test(b.username)).slice(0, 10);
    ...
    <div className="flex flex-col gap-8">
      <header>
        ...
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted">
          Liorin plays White in cold mode — academy book, win-seek, no mate-hangs. The field
          dummy across the board is a weaker sparring engine. Live BOT names are labels, not
          Lc0. Liorin does not yield. The score updates Conquest Elo and the academy archive.
        </p>
      </header>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,520px)_1fr]">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              onClick={() => {
                setEngineSalt((Date.now() ^ opponent.length * 2654435761) >>> 0);
                setRunning(true);
              }}
            ...
            <Button
              ...
              onClick={() => {
                ...
                setLastMove(null);
                setRunning(false);
                setEngineSalt((Date.now() ^ 0x4c494f52) >>> 0);
                setLog([`Reset vs ${opponent}. New line.`]);
              }}
            >
              Reset
            </Button>
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-lg border border-border bg-surface p-4">
            ...
            <p className="mt-2 text-sm text-fg">
              Liorin (White, cold) vs {opponent} ({formatElo(oppRating)}, field dummy)
            </p>
            {record.data ? (
